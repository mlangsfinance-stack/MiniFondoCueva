...
|---|---|---|---|---|
| 0–10 | **Observación** | Una idea, de donde venga | Se puede escribir en una frase qué hace el mercado después de qué | Nota en `hipotesis/REGISTRO.md` (estado `idea`) |
| 10–25 | **Hipótesis** | Idea escrita | Ficha completa: mecanismo, universo, predicción, métrica, criterio de muerte | `hipotesis/Hxxx_nombre.md` |
...
| 40–55 | **Prototipo** | Tendencia confirmada | Señal ≤ 30 líneas, ≤ 4 parámetros · con costes: PF > 1.2 · ≥ 100 trades · expectancia > 0.1 R · t-stat > 2 | Función en `src/quantlab/senales.py` (o módulo propio) + métricas |
| 55–75 | **Validación** | Prototipo que pasa | Las 5 fases en verde ([03_VALIDACION.md](03_VALIDACION.md)) | `reportes/<nombre>/RESUMEN.md` |
| 75–85 | **Estrategia** | Validada | Ficha completa: sizing, límites, correlación con lo existente, plan de ejecución, plan de retirada | `estrategias/4_incubacion/Exxx_nombre.md` |
...
- **El registro es sagrado.** Cada test, pase o no, es una fila en `hipotesis/REGISTRO.md`. Ese
...
Hipótesis     → hipotesis/_plantilla.md
Tendencia     → scripts/01_tendencia.py   quantlab.tendencies (estudio_evento, prob_rango, test_permutacion)
Prototipo     → scripts/02_prototipo.py   quantlab.senales + quantlab.backtest + quantlab.metrics
Validación    → scripts/03_validar.py     quantlab.validation (5 fases) + quantlab.report
Estrategia    → estrategias/_plantilla.md (sizing, cartera, ejecución, retirada: docs/04)
